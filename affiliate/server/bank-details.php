  ...
  if(!empty($account_number) && !empty($bank) && !empty($bank_code) && !empty($currency) && !empty($fullname)){
    ...
    if ($err) {
        $data = array("Info" => "Error connecting to Paystack gateway");   
          ...
          if ($status === true) {
            $update_query = mysqli_query($conn, "UPDATE affiliates SET account_number = '$account_number', bank = '$bank', bank_code = '$bank_code', recipient_code = '$recipient_code' WHERE affiliateID = '$affiliateID'");
