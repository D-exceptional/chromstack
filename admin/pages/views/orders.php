          ...
          <table class="table table-striped projects">
            ...
            <tbody>
                <?php 
                  ...
                  $sql = mysqli_query($conn, "SELECT * FROM purchased_courses ORDER BY purchase_date DESC");
                  if (mysqli_num_rows($sql) > 0) {
                    while($result = mysqli_fetch_assoc($sql)){
                      ...
                      if ($status == 'Pending') {
                          $status_button = "<button class='btn btn-danger btn-sm'>$status</button>";
                          $action_button = "<button class='btn btn-info btn-sm'>View</button>";
                      }
                      else {
                          $status_button = "<button class='btn btn-success btn-sm'> $status</button>";
                          $action_button = "<button class='btn btn-info btn-sm'>View</button>";
                      }
...
<script src="<?php echo getCacheBustedUrl('../scripts/orders.js'); ?>" type="module"></script>
</body>
</html>