<?php 
  ...
  //Get savings if any
  $savings_query = mysqli_query($conn, "SELECT SUM(wallet_amount) AS total_savings FROM wallet WHERE wallet_email = '$email' AND wallet_user = 'Admin'");
...
<head>
  ...
  <title>Admin | Weekly Withdrawal</title>
        ...
        <div class="card-header">
          ...
          <div class="card-tools">
            <?php
              ...
              if ($currentDay === 'Thursday' && $currentHour <= 23) { //Withdrawal should be done before 8pm
        ...
        <div class="card-body p-0" style='overflow-x: auto !important;white-space: normal;'>
          <table class="table table-striped projects">
              ...
              <tbody>
                ...
                <?php 
                  $history_query = mysqli_query($conn, "SELECT * FROM withdrawals WHERE withdrawal_email = '$email' ORDER BY withdrawalID ASC");
                  if(mysqli_num_rows($history_query) > 0){
                    while($value = mysqli_fetch_assoc($history_query)){
                      $button = '';
                      $id = $value['withdrawalID'];
...
<script src="<?php echo getCacheBustedUrl('../scripts/wallet.js'); ?>" type='module'></script>
