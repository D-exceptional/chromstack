    ...
    success: function (response) {
      for (var key in response) {
        if (Object.hasOwnProperty.call(response, key)) {
          ...
          if (
            content !== "Error updating details" &&
        ...
        if (Object.hasOwnProperty.call(response, key)) {
          ...
          if (
            ...
            $(".contest-list").each(function () {
              if ($(this).attr("id") == id) {
                $(this)
                  .find(".contest-description")
                  .attr("contenteditable", false)
                  .empty()
                  .text(response.details.description);
                $("#button-div button").empty().text("Edit");
              }
            });
            displaySuccess(content);
          } else {
...
function fetchActiveContests() {
  $("tbody").empty();
  $.ajax({
    type: "GET",
    url: "../server/active-contests.php",
    data: { id: userID },
    dataType: "json",
    success: function (response) {
      ...
      $(".col-sm-6 h1")
        .empty()
        .html("<b>Active Contests " + "(" + counter + ")" + "</b>");
              ...
              switch (buttonText) {
                ...
                case "Stats":
                  ...
                  $("#full-details-overlay iframe").attr(
                    "src",
                    `../../../contest-statistics.php?id=${contestID}`
                  );
                  break;
                case "Link":
                   ...
                   setTimeout(() => {
                     $(el).empty().html("Link").attr("disabled", false);
                   }, 2000);
                  break;
              }
            });
