<?php
    ...
    $admin_commission = (100 - intval($real_affiliate_commission)) . '%';
                        ...
                        foreach($files as $file){
                            if(!is_dir($file)){
                                ...
                                if (in_array($name_info, ['cover_page', 'cover-page', 'course_cover_page', 'course-cover-page'])) {
                                    ...
                                    $insert_query = mysqli_query($conn, "INSERT INTO affiliate_program_course (course_title, course_description, course_cover_page, course_type, course_status, course_authors, course_amount, sales_page, course_category, course_narration, admin_percentage, affiliate_percentage, uploaded_on, folder_path)

                                    VALUES ('$title', '$description', '$course_cover_image', '$course_type', '$status', '$fullname', '$amount', '$sales_page', '$category', '$product_format', '$admin_commission', '$affiliate_commission', '$date', '$foldername')");
                                    ...
                                    if($insert_query === true){ 
                                        ...
                                        $notification_title = 'uploaded affiliate main course';
                                        $notification_details = 'The main course with the title ' . '<b>' . $title . '</b>' . ' was uploaded';
