<?php
 ...
 require 'conn.php';
    ...
    if(!empty($currentPassword) && !empty($newPassword)){
          ...
          $check = mysqli_query($conn, "SELECT account_password FROM admins WHERE adminID = '$adminID'");
          $value = mysqli_fetch_assoc($check);
          $dbPassword = $value['account_password'];

          if (password_verify($currentPassword, $dbPassword)) {

               $update_query = mysqli_query($conn, "UPDATE admins SET account_password = '$hashPassword' WHERE adminID = '$adminID'");
