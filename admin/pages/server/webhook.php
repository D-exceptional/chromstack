<?php
  ...
  if ((strtoupper($_SERVER['REQUEST_METHOD']) != 'POST' ) || !array_key_exists('HTTP_X_PAYSTACK_SIGNATURE', $_SERVER) ) 
    ...
    define('PAYSTACK_SECRET_KEY', 'sk_live_dd519ff2272708c948e5e92b4149029ab52328ca');
    ...
    require 'verification-functions.php';
    ...
    switch ($event_type) {
      ...
      case 'charge.success':
        ...
        $query = mysqli_query($conn, "SELECT * FROM affiliate_course_sales WHERE sales_txref = '$reference' AND sales_status = 'Pending'");
        if(mysqli_num_rows($query) > 0){
           verify_affiliate_course_purchase($reference);
        }

        $query = mysqli_query($conn, "SELECT * FROM uploaded_course_sales WHERE sales_txref = '$reference' AND sales_status = 'Pending'");
        if(mysqli_num_rows($query) > 0){
           verify_course_purchase($reference);
        }

        $query = mysqli_query($conn, "SELECT * FROM membership_payment WHERE payment_ref = '$reference' AND payment_status = 'Pending'");
        if(mysqli_num_rows($query) > 0){
      ...
      case 'transfer.success':
        ...
        $payout_amount = ($amount / 100) / 500;
        $rounded_payment = round($payout_amount, 2); 
        //Update record on database
        update_transaction($reference, $status, $recipient_name, $rounded_payment);
      break;
