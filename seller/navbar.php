    ...
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item dropdown">
        ...
        <?php 
         ...
         <!-- Check course orders --->
         <?php 
            $sales_count = mysqli_query($conn, "SELECT * FROM general_notifications WHERE notification_type = 'course_purchase' AND notification_receiver_email = '$email' AND notification_status = 'Unseen'");
            if (mysqli_num_rows($sales_count) > 0) {
               $total = mysqli_num_rows($sales_count);
               //Get last notification item time
               $last_time = mysqli_query($conn, "SELECT notification_date FROM general_notifications WHERE notification_type = 'course_purchase' AND notification_receiver_email = '$email' AND notification_status = 'Unseen' ORDER BY notificationID DESC LIMIT 1");
               $value = mysqli_fetch_assoc($last_time);
               $notification_date = $value['notification_date'];
               $counter = timeAgo($notification_date);
               echo "<a href='#' class='dropdown-item'>
