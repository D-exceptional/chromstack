...
$(document).ready(function () {
    ...
    //Check zip file
      function previewCourseFile(input) {
        if (input.files && input.files[0]) {
          ...
          switch (extension) {
              ...
              case 'jfif':
                  $('#course-file-name').css({'display':'block'}).empty().text(name);
                  displayInfo('Selected file format not supported. Choose a file format with .zip extension !');
      ...
      $('#course-main-file').on('change', function () {
    ...
    $("#course-upload-form").on("submit", function (event) {
        ...
        if ($('#course-main-file').val() !== '') {
            if ( $('#title').val() !== '' && $('#description').val() !== '' && $('#amount').val() !== '' && $('#commission').val() !== '') {
              //Upload form
