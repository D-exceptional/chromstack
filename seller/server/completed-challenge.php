...
$sql = mysqli_query($conn, "SELECT courseID FROM uploaded_courses WHERE course_authors = '$vendor' AND course_status = 'Approved'");
...
if(mysqli_num_rows($sql) > 0){
    ...
    while($row = mysqli_fetch_assoc($sql)){
        ...
        $query = mysqli_query($conn, "SELECT * FROM contest WHERE courseID = '$courseID' AND contest_status = 'Completed'");
