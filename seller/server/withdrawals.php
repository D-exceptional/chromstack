  ...
  if (!empty($name) && !empty($email) && !empty($amount) && !empty($bank) && !empty($narration) && !empty($balance)) {
    ...
    if (mysqli_num_rows($check) > 0) {
      ...
      if ($insertWithdrawal) {
        ...
        $sql = mysqli_query($conn, "SELECT * FROM wallet WHERE wallet_email = '$email' AND wallet_user = 'Vendor'");
        if (mysqli_num_rows($sql) > 0) {
          // Update the existing record if found
          if (mysqli_query($conn, "UPDATE wallet SET wallet_amount = '$balance' WHERE wallet_email = '$email' AND wallet_user = 'Vendor'") === true) {
            // Prepare response
          ...
          if (mysqli_query($conn, "INSERT INTO wallet (wallet_email, wallet_amount, wallet_user) VALUES ('$email', '$balance', 'Vendor')") === true) {
            // Prepare response
            $response = array('Info' => "Withdrawal request placed successfully.");
            // Send confirmatory email to affiliate
            $subject = "Successful Withdrawal Request";
