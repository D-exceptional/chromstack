    ...
    switch ($user_type) {
        case 'User':
            ...
            if(mysqli_num_rows($sql) > 0){
                if(mysqli_query($conn, "UPDATE users SET user_status = 'Active' WHERE email = '$email'") === true){
                    $type = '<i class="fas fa-check" style="color: green;"></i>';
                    $status = 'Activation Successful';
                    $message = "
                                Your account has been activated successfully.
                                For maximum user experience, <a href='https://chromstack.com/login'>login</a> to your account.
                               ";
                }
            }
            else{ 
                $type = '<i class="fas fa-exclamation" style="color: red;"></i>';
                $status = 'Activation Failed';
                $message = 'No info found for user';
            } 
        break;
        case 'Affiliate':
            $sql = mysqli_query($conn, "SELECT * FROM affiliates WHERE email = '$email'");
            if(mysqli_num_rows($sql) > 0){
                if(mysqli_query($conn, "UPDATE affiliates SET affiliate_status = 'Active' WHERE email = '$email'") === true){
                    $type = '<i class="fas fa-check" style="color: green;"></i>';
                    $status = 'Activation Successful';
                    $message = "
                                Your account has been activated successfully.
        ...
        break;
        case 'Vendor':
            $sql = mysqli_query($conn, "SELECT * FROM vendors WHERE email = '$email'");
            if(mysqli_num_rows($sql) > 0){
               if(mysqli_query($conn, "UPDATE vendors SET vendor_status = 'Active' WHERE email = '$email'") === true){
                    $type = '<i class="fas fa-check" style="color: green;"></i>';
                    $status = 'Activation Successful';
                    $message = "
                                Your account has been activated successfully.
...
<head>
    <meta charset="utf-8">
    <title>Chromstack | Activation</title>
        ...
        <div class="container">
            <div class="row g-4" style="display: flex;flex-direction: column;align-items: center;justify-content: center;">
                ...
                <div class="col-lg-6 col-md-6 wow fadeInUp" data-wow-delay="0.1s">
                    <h5 style="text-align: center;"><?php echo $status; ?></h5>
            ...
            <div class="container py-5">
                <div class="row g-5">
                    <div class="col-lg-4 col-md-6">
                        ...
                        <a class="btn btn-link" href="ticket-list">Tickets</a>
